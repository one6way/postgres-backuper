...
# Инициализация DAG
dag = DAG(
    'kafka_to_adb_spark_cert_manager',
    default_args=default_args,
    description='Transfer data from Kafka to ADB using Spark with cert-manager integration',
    schedule_interval='@daily',
    start_date=days_ago(1),
    catchup=False,
    tags=['spark', 'kafka', 'adb', 'cert-manager'],
)
...
def create_spark_task():
    return KubernetesPodOperator(
        ...
        # Монтирование секретов, созданных cert-manager
        secrets=[
            {
                "secret": "kafka-client-certs",  # Имя секрета, созданного cert-manager
