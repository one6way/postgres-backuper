...
def setup_vault_env():
    """Настройка переменных окружения из Vault"""
    try:
        # Проверяем наличие файла с паролями от Vault Agent Injector
        passwords_file = '/vault/secrets/kafka-passwords'
        if os.path.exists(passwords_file):
            logger.info(f"Загрузка переменных окружения из {passwords_file}")
            with open(passwords_file, 'r') as f:
                for line in f:
                    if line.strip() and '=' in line:
                        key, value = line.strip().replace('export ', '').split('=', 1)
                        os.environ[key] = value
                        logger.info(f"Установлена переменная {key}")
        else:
            logger.warning(f"Файл с паролями не найден: {passwords_file}")
    except Exception as e:
        logger.error(f"Ошибка при настройке переменных окружения из Vault: {str(e)}")
        raise

def create_spark_session():
    """Создание и настройка SparkSession"""
    try:
        # Получаем пути к сертификатам из переменных окружения
        keystore_location = os.environ.get("KAFKA_KEYSTORE_PATH", "/vault/secrets/kafka-keystore.jks")
        truststore_location = os.environ.get("KAFKA_TRUSTSTORE_PATH", "/vault/secrets/kafka-truststore.jks")
